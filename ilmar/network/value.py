...
class StateActionConcatedFunction(nn.Module):
    """
    Value function that takes s-a pairs as input
...
class AdvantageStateActionFunction(nn.Module):
    def __init__(
            self,
            state_shape: np.array,
            action_shape: np.array,
        ...
        self.net1 = build_mlp(
            input_dim=state_shape[0],
            output_dim=hidden_units[0],
            hidden_units=(),
            hidden_activation=hidden_activation
        )
        self.net2 = build_mlp(
            input_dim=action_shape[0],
            output_dim=hidden_units[0],
            hidden_units=(),
            hidden_activation=hidden_activation
        )
        self.net3 = build_mlp(
            input_dim=3*hidden_units[0],
            output_dim=1,
            hidden_units=hidden_units,
            hidden_activation=hidden_activation
        )

    def forward(self, states: torch.Tensor, actions_1: torch.Tensor,actions_2: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        values_2: torch.Tensor
            values of the s-a pairs
        """
        states_embbings= self.net1(states)
        actions_1_embbings = self.net2(actions_1)
        actions_2_embbings = self.net2(actions_2)
        xs = torch.cat([states_embbings, actions_1_embbings,actions_2_embbings], dim=-1)
        return self.net3(xs)
