...
def collect_d4rl(
    ...
    """
    Collect demonstrations using the well-trained policy and save to D4RL format
    ...
    p_rand: float
        with probability of p_rand, the policy will act randomly
    env_id: str
        environment ID to be used for saving the dataset
    seed: int
        random seed
    ...
    for _ in tqdm(range(1, buffer_size + 1)):
        ...
        if np.random.rand() < p_rand:
            ...
            # if True:
            #     action = algo.exploit((state+ shift) * scale)
            # else:
            if True:
                action = algo.exploit(state)
            action = add_random_noise(action, std)
    ...
    save_dir = os.path.join('datasets', env_id)
    os.makedirs(save_dir, exist_ok=True)
    # Save dataset in D4RL format
    save_path = os.path.join(save_dir, f'{mean_return}.hdf5')
    # Save dataset in D4RL format
    with h5py.File(save_path, 'w') as f:
        ...
        f.attrs['env'] = env_id  # Environment name
