...
class Trainer:
    ...
    def __init__(
        ...
        self.plot_dir= os.path.join(self.config["plots_dir"], env.spec.id)
        # Other parameters.
    ...
    def train(self):
        ...
        best_reward=0
        for step in tqdm(range(0, self.num_steps+1)):
            ...
            if self.algo_type:
                ...
                self.algo.update(self.writer,self.plot_dir)

            # Evaluate regularly.
            if step % self.eval_interval == 0:
                rewards = self.evaluate(step)
                if self.algo_type:
                    self.algo.save_models(os.path.join(self.model_dir, f'step{step}'))
                if self.config["save_best"]:
                    if rewards > best_reward:
                        self.algo.save_models(os.path.join(self.logger.log_dir, f'step{step}', f'reward{int(rewards)}'))
    ...
    def evaluate(self, step: int):
        ...
        for _ in range(self.num_eval_episodes):
            state = self.env_test.reset(seed=_)[0]
            episode_return = 0.0
            done = False
            t = 0
        ...
        tqdm.write(f'Num steps: {step:<6}, '
                   ...
                   f'Time: {self.time}')
        wandb.log({"Return": np.mean(returns)},step=step)
