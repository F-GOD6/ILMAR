...
class SerializedBuffer:
    """
    Serialized buffer, containing [states, actions, rewards, done, next_states,init_states]
    ...
    def __init__(
        ...
        self.init_states = tmp['init_state'].clone().to(self.device)
        self.traj_states = []
        self.traj_actions = []
        self.traj_rewards = []
...
class Buffer(SerializedBuffer):
    ...
    def __init__(
        ...
        self.next_states = torch.empty(
            (buffer_size, *state_shape), dtype=torch.float, device=device)
        self.init_states = torch.empty(
            (buffer_size, *state_shape), dtype=torch.float, device=device)

    def append(self, state: np.array, action: np.array, reward: float, done: bool, next_state: np.array,init_states: np.array):
        """
        ...
        self.states[self._p].copy_(torch.from_numpy(state))
        self.init_states[self._p].copy_(torch.from_numpy(state))
    ...
    def save(self, path: str):
        ...
        torch.save({
            ...
            'init_state':self.init_states.clone().cpu(),
        }, path)
...
class Buffer_init(SerializedBuffer):
    """
    Buffer used while collecting demonstrations
    ...
    def __init__(
        ...
        self.init_states = torch.zeros((0, *state_shape), dtype=torch.float, device=device)

    def append(self, init_states: np.array):
        """
        Save a transition in the buffer
        ...
        """
        # self.states[self._p].copy_(torch.from_numpy(state))
        self.init_states = torch.cat((self.init_states, torch.unsqueeze(torch.from_numpy(init_states).to(self.device), 0)), dim=0)
        self._p = (self._p + 1) % self.buffer_size
        self._n = min(self._n + 1, self.buffer_size)
    ...
    def save(self, path: str):
        ...
        torch.save({
            'init_states': self.init_states.clone().cpu()
        }, path)


class SerializedBuffer_init:
    """
...
def sample_from_dataset(dataset,batch_size,device):
    init_indices = np.random.randint(0, len(dataset["init_states"]), size=batch_size)
    indices = np.random.randint(0, len(dataset["states"]), size=batch_size)
    states, actions, next_states, init_states, rewards, dones =  torch.from_numpy(dataset["states"][indices]).to(device), \
         torch.from_numpy(dataset["actions"][indices]).to(device), torch.from_numpy(dataset["next_states"][indices]).to(device), \
            torch.from_numpy(dataset["init_states"][init_indices]).to(device),torch.from_numpy(dataset["rewards"][indices]).to(device), \
                torch.from_numpy(dataset["dones"][indices]).to(device)
    return states, actions, next_states, init_states, rewards, dones
